...
import com.neu.youthpathtalk.compensate.message.CommonCompensateMessage;
...
@RocketMQMessageListener(
        ...
        consumerGroup = MQConstants.CONSUMER_GROUP_USER_INTERACTION_HISTORY_COMPENSATE,
        consumeMode = ConsumeMode.ORDERLY
)
public class UserLikeHistCompConsumer implements RocketMQListener<CommonCompensateMessage> {
    private final RedisService redisService;

    @Override
    public void onMessage(CommonCompensateMessage message) {
        log.info("收到用户维度点赞记录补偿消息:{}",message);
        String likeHistoryKey= UserRedisKey.likeHistory(message.getUserId());
        try {
            if (Boolean.TRUE.equals(message.getAdd())){
            ...
            log.info("补偿消息处理成功: userId={}, postId={}, add={}",
                    message.getUserId(),message.getPostId(),message.getAdd());
        } catch (Exception e) {
