...
public interface IdempotentMapper {
    @Insert("INSERT IGNORE INTO user_like_count_msg_idempotent (message_id) VALUES (#{messageId})")
    int insertIfNotExist(@Param("messageId") String messageId);
    @Delete("DELETE FROM user_like_count_msg_idempotent WHERE create_time < #{threshold} LIMIT #{batchSize}")
